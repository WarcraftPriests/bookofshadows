...
## Sim Charts
As of this post our sims posted to the [Charts Website](https://warcraftpriests.github.io) now use a new sim known as Dungeon Composite. This sims every key at a high level for that season and combines them together to form a single chart. This gives you generic ideas of what to target/shoot for more overall dungeon dps. We also have result tables built for every sim chart for every individual dungeon. Right now those are not visible on the charts website but might be in the future if we figure out a good way to display things. For now you can navigate to the [GitHub Repo](https://github.com/WarcraftPriests/mid-shadow-priest), select any sim you are interested in and go to the `results/dungeons` folder to find all the specific dungeon sims.

The routes we use for these sims are found [here](https://github.com/WarcraftPriests/mid-shadow-priest/tree/master/internal/routes). See below on how this works.
...
1. Import or create a route for the key you are interested in simming in [Keystone Guru](https://keystone.guru/). Make sure these routes include accurate pulls and include bosses. If you fail to select something it will not be simmed. This also means if you do not create individual pulls it will sim it as if you pull it all at once.
2. Press the `Simulate` button at the top right of any route page.
3. Select whatever Raid buffs and the Keystone level you would like. Then select `Get SimulationCraft string`
4. Copy the output to your clipboard and then paste that below your `/simc` string, likely in advanced mode in Raidbots.

Note as mentioned above you can also use some routes we generated [here](https://github.com/WarcraftPriests/mid-shadow-priest/tree/master/internal/routes) instead of doing that yourself.
